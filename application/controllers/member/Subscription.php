...
class Subscription extends CI_Controller{
    ...
    function index($start=0){
		$limit = 10;
        $data['subscription'] = $this->Subscription_model->get_limit_users_subscription($limit,$start);
		//pagination
		$config['base_url'] = site_url('member/subscription/index');
		$config['total_rows'] = $this->Subscription_model->get_count_users_subscription();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'member/subscription/index';
...
'subscriptionid' => html_escape($this->input->post('subscriptionid')),
...
'updated_at' =>$updated_at,
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('member/subscription/index');
            }else{
                $data['_view'] = 'member/subscription/form';
                $this->load->view('layouts/member/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['subscription'] = $this->Subscription_model->get_subscription($id);
		$data['id'] = $id;
        $data['_view'] = 'member/subscription/details';
        $this->load->view('layouts/member/body',$data);
	}
...
$this->db->or_like('subscriptionid', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('member/subscription/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('users_id', $key, 'both');
$this->db->or_like('subscriptionid', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   $header = array("Id","Users Id","Subscriptionid","Service Type Id","Activation Key","Limit Count","Used Count","Expiration Date Time","Created At","Updated At"); 
		   fputcsv($file, $header);
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/member/subscription/print_template.php');
			$html = ob_get_clean();
