...
class Subscription_history extends CI_Controller{
    ...
    function index($start=0){
		$limit = 10;
        $data['subscription_history'] = $this->Subscription_history_model->get_limit_subscription_history($limit,$start);
		//pagination
		$config['base_url'] = site_url('admin/subscription_history/index');
		$config['total_rows'] = $this->Subscription_history_model->get_count_subscription_history();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'admin/subscription_history/index';
...
'updated_at' =>$updated_at,
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('admin/subscription_history/index');
            }else{
                $data['_view'] = 'admin/subscription_history/form';
                $this->load->view('layouts/admin/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['subscription_history'] = $this->Subscription_history_model->get_subscription_history($id);
		$data['id'] = $id;
        $data['_view'] = 'admin/subscription_history/details';
        $this->load->view('layouts/admin/body',$data);
	}
...
$this->db->or_like('updated_at', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('admin/subscription_history/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('subscriptionid', $key, 'both');
$this->db->or_like('activation_key', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/admin/subscription_history/print_template.php');
			$html = ob_get_clean();
