...
'service_email' => html_escape($this->input->post('service_email')),
'service_password' => html_escape($this->input->post('service_password')),
'used' => html_escape($this->input->post('used')),
'used_by_users_id' => html_escape($this->input->post('used_by_users_id')),
'used_by_activation_key' => html_escape($this->input->post('used_by_activation_key')),
	...
	function import_excell()
	 {
		...
		if(isset($_FILES["file"]["name"]))
		{
			...
			for($row=1; $row<=count($sheet_data); $row++)
		    {
				...
				$service_email  = isset($sheet_data[$row][1])?$sheet_data[$row][1]:'';
				$service_password = isset($sheet_data[$row][2])?$sheet_data[$row][2]:'';
...
$this->db->or_like('service_email', $key, 'both');
$this->db->or_like('service_password', $key, 'both');
$this->db->or_like('used', $key, 'both');
$this->db->or_like('used_by_users_id', $key, 'both');
$this->db->or_like('used_by_activation_key', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   $header = array("Id","Service Type Id","Service Email","Service Password","Used","Used By Users Id","Used By Activation Key","Created At","Updated At"); 
		   fputcsv($file, $header);
